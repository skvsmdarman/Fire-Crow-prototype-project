...
import { SystemStatusViewModel } from "../../features/system/types";
import { formatDateTime, shortRepoName } from "../../shared/utils/format";
import { LogLine } from "../../shared/hooks/useSSE";
import { GITHUB_SCOPE_DESCRIPTIONS, PRODUCT_NAME, PRODUCT_TAGLINE } from "../../shared/config/app";
  ...
  return (
    <aside className={styles.sidebar}>
      ...
      <div className={styles.workspaceCard}>
        ...
        <div className={styles.workspaceName}>{workspace || "acme-corp"}</div>
        <div className={styles.workspaceId}>{userId || "usr_unknown"}</div>
      </div>
      <button onClick={onSignOut} className={styles.ghostAction}>
        Sign out
      </button>
    </aside>
  );
        ...
        <div className={styles.workGrid}>
          ...
          <div className={styles.panel}>
            <div className={styles.panelHeader}>
              ...
              <button onClick={() => setActive("Audits")} className={styles.ghostAction}>
                View all →
              </button>
            </div>
            {jobs.length === 0 ? (
                ...
                {jobs.slice(0, 4).map((j) => (
                  <div key={j.id} className={styles.jobRow}>
                    <div className={styles.jobInfo}>
                      <svg className={styles.jobIcon} width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                      ...
                      <div className={styles.jobMetaBlock}>
                        ...
                        <small>
                          {j.repo_branch} · {formatDateTime(j.created_at)}
                        </small>
                      </div>
                    </div>
          ...
          <div className={styles.panel}>
            ...
            <div className={styles.panel} style={{ border: "none", padding: 0 }}>
              {findings.length === 0 ? (
                <div className={styles.emptyState}>No findings available.</div>
              ) : (
                  ...
                  return (
                    <div key={sev} style={{ marginBottom: 12 }}>
                      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                        <span className="mono" style={{ fontSize: 10, textTransform: "uppercase", letterSpacing: "0.08em", color }}>
                          {sev}
                        </span>
...
export function AuditLaunchPanel({
  newUrl,
  setNewUrl,
  newBranch,
  setNewBranch,
  ...
  setAttestationAccepted: (b: boolean) => void;
  authorizationScope: string;
  setAuthorizationScope: (s: string) => void;
  submitting: boolean;
  ...
  return (
    <div className={styles.panel} style={{ marginBottom: 16 }}>
      ...
      <form onSubmit={onSubmit} className={styles.auditForm}>
        <div>
          <span className={styles.sectionKicker} style={{ fontSize: 9, marginBottom: 6, display: "block" }}>
            Repository URL
          </span>
          <input
            value={newUrl}
        ...
        <div style={{ marginTop: 8 }}>
          ...
          >
            ...
            <option value="owner">Repository Owner</option>
            <option value="open_source_contributor">Open Source Contributor (public repo)</option>
          </select>
        </div>
        <div style={{ padding: "12px", background: "rgba(255,184,0,0.06)", border: "1px solid rgba(255,184,0,0.15)", borderRadius: 10, fontSize: 11, color: "var(--amber)", lineHeight: 1.6 }}>
          <label style={{ display: "flex", gap: 10, alignItems: "flex-start", cursor: "pointer" }}>
            ...
            />
            <span>
              I attest that I have explicit authorization to perform security audits on this repository under national IT security policies.
            </span>
          </label>
        </div>
        <button
          type="submit"
          disabled={submitting || !newUrl.trim() || !attestationAccepted}
          className={styles.submitButton}
          style={{
            opacity: submitting || !newUrl.trim() || !attestationAccepted ? 0.5 : 1,
            cursor: submitting || !newUrl.trim() || !attestationAccepted ? "not-allowed" : "pointer",
          }}
        >
...
export function AuditJobList({
  ...
  return (
    <div className={styles.panel}>
      ...
      {jobs.length === 0 ? (
        <div className={styles.emptyState}>No audits.</div>
      ) : (
        <div className={styles.jobList}>
          {jobs.map((j: Job) => (
            <button
              ...
              className={`${styles.jobRow} ${selectedId === j.id ? styles.jobRowActive : ""}`}
            >
              {selectedId === j.id && <span className={styles.activeJobIndicator} />}
              <div className={styles.jobInfo}>
                <svg className={styles.jobIcon} width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                  <path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71" />
                  <path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71" />
...
export function AuditJobDetail({
  selected,
  insight,
  downloadReport,
  downloadingId,
  ...
  selected: Job;
  insight: { enabled: boolean; insight: string | null } | null;
  downloadReport: (jobId: string, repoUrl: string) => Promise<void>;
    ...
    <div className={styles.panel}>
      ...
      <div style={{ display: "grid", gap: 12 }}>
        ...
        <div className={styles.auditStatusStrip}>
          <span className={`mono ${styles.sectionKicker}`} style={{ width: "100%", marginBottom: 4 }}>
            Pipeline
          </span>
        ...
        {insight?.enabled && insight.insight && (
          <div style={{ marginBottom: 16, padding: "12px 14px", background: "rgba(255, 107, 43, 0.08)", border: "1px solid rgba(255, 107, 43, 0.18)", borderRadius: 6 }}>
            <div className="mono" style={{ fontSize: 9, color: theme.orange, letterSpacing: "0.12em", textTransform: "uppercase", marginBottom: 8 }}>
              AI Insight
            </div>
            <div style={{ fontSize: 13, lineHeight: 1.5, color: theme.text }}>{insight.insight}</div>
            <div style={{ fontSize: 11, color: theme.muted, marginTop: 8 }}>
              Optional LLM hint only. Findings and report evidence remain the source of truth.
            </div>
          </div>
        )}
        ...
        {selected.report_pdf_url && (
          <button
            onClick={() => downloadReport(selected.id, selected.repo_url)}
          ...
          >
            {downloadingId === selected.id ? "Downloading report..." : "Download PDF report ↓"}
          </button>
        )}
        ...
        {selected.status === "completed" ? (
          <div style={{ marginTop: 20 }}>
            <div className="mono" style={{ fontSize: 9, color: theme.muted, letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: 10 }}>
              Attack Graph
            </div>
                  ...
                  <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.25 }}>
                    <div className={styles.findingDetailPanel}>
                      <div className={styles.findingDetailContent}>
                        ...
                        <div className={styles.detailSection}>
                          <h4>Evidence</h4>
                          <p className={styles.evidenceBlock} style={{ margin: 0, fontSize: 12, lineHeight: 1.6 }}>
                            {f.evidence || "No evidence snippet provided."}
                          </p>
                        </div>
                      </div>
                    </div>
    ...
    <div>
      ...
      <div className={styles.sectionBody}>
        <div className={styles.reportList}>
          {jobs.length === 0 ? (
            <div className={styles.emptyState}>No completed audits yet.</div>
          ) : (
            jobs.map((j) => (
              <div key={j.id} className={styles.reportRow}>
                <div>
                  ...
                  <p>
                    Branch {j.repo_branch} · finished {j.finished_at ? formatDateTime(j.finished_at) : "-"}
                  </p>
                </div>
                {j.report_pdf_url ? (
                  <button
                    onClick={() => downloadReport(j.id, j.repo_url)}
                    disabled={downloadingId === j.id}
                    className={styles.ghostAction}
                    style={{ cursor: "pointer" }}
                  >
                    {downloadingId === j.id ? "Opening..." : "Open report ↓"}
                  </button>
                ) : (
...
export function SettingsPanel({ systemStatus }: { systemStatus: SystemStatusViewModel }) {
  const { toast } = useToast();
  const [dbStats, setDbStats] = useState<DbStats | null>(null);
  const [loadingStats, setLoadingStats] = useState(false);
  const [pruning, setPruning] = useState(false);

  const isAdmin = systemStatus.integrations !== "admin_only" && systemStatus.integrations !== "unknown" && systemStatus.integrations !== null;
    ...
    <div>
      ...
      <div className={styles.sectionBody} style={{ maxWidth: 640 }}>
        ...
        <div className={styles.panel}>
          ...
          {[
            ["API", systemStatus.api, systemStatus.api === "online" ? theme.green : theme.amber],
            ["Database", systemStatus.database, systemStatus.database === "connected" ? theme.green : theme.amber],
            ["Sandbox", systemStatus.sandbox_mode, theme.text],
          ].map(([l, v, c]) => (
            <div key={l as string} style={{ padding: "14px 18px", borderBottom: `1px solid ${theme.border}`, display: "flex", justifyContent: "space-between" }}>
              <span className="mono" style={{ fontSize: 11, color: theme.muted, textTransform: "uppercase", letterSpacing: "0.1em" }}>
                {l}
              </span>
        ...
        <div className={styles.statusCard}>
          ...
          {systemStatus.integrations === "unknown" || !systemStatus ? (
            <div className={styles.emptyState} style={{ border: "none" }}>
              Loading integrations...
            </div>
          ) : systemStatus.integrations === "admin_only" ? (
            <div className={styles.emptyState} style={{ border: "none" }}>
              Only accessible to administrators.
            </div>
          ) : (
            <div className={styles.integrationList}>
              {Object.entries(systemStatus.integrations).map(([l, val]) => (
                <div key={l} className={styles.integrationRow}>
                  <span>{l.replace("_", " ")}</span>
                  <strong className={val === "configured" ? styles.integrationOn : styles.integrationOff}>
                    {String(val)}
                  </strong>
                </div>
              ))}
        ...
        <div className={styles.statusCard}>
          ...
          {systemStatus.scanner_capabilities === "not_reported" ? (
            <div className={styles.emptyState} style={{ border: "none" }}>
              Scanner capabilities not available.
            </div>
          ) : (
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16, marginTop: 8 }}>
            <div className={`mono ${styles.sectionKicker}`} style={{ fontSize: 13, display: "flex", alignItems: "center", gap: 8 }}>
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="var(--orange)" strokeWidth="2">
                ...
                <path d="M22 7c0 2.761-4.477 5-10 5S2 12 2 7" />
              ...
              {dbStats?.row_counts ? (
                Object.entries(dbStats.row_counts).map(([tbl, cnt]) => (
                  <div key={tbl} className={styles.integrationRow}>
                    ...
                    <span className="mono" style={{ fontSize: 11, fontWeight: 600, color: "var(--text)", background: "var(--dim)", padding: "2px 8px", borderRadius: 4 }}>
                      {cnt}
                    </span>
                  </div>
                ))
              ) : (
                <div className={styles.emptyState}>{loadingStats ? "Loading table stats..." : "No registry data loaded."}</div>
              )}
            </div>
            ...
            <div className={styles.panel} style={{ borderColor: "var(--orangeBorder)" }}>
              ...
              >
                <svg
                  ...
                  style={{ animation: pruning ? "spin 1.5s linear infinite" : "none" }}
                >
