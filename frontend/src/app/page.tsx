...
const FIRST_RUN_HINTS = [
  {
    ...
    body: "A smaller service, staging branch, or recent hotfix usually makes the first audit easier to verify.",
  },
  {
    title: "Treat the first report like calibration",
    body: "The first pass should start a useful conversation. Teams can tighten scope after reviewing one real run.",
  },
  {
    title: "Pull one engineer into the first review",
    body: "Findings move faster when the reviewer can map the trace back to code they understand.",
  },
];
...
const CAPABILITIES = [
  {
    ...
    stat: "Traceable",
    body: "Every audit keeps agent notes, confirmed findings, and report artifacts in one review surface.",
  },
  {
    title: "Safe runtime validation",
    stat: "Bounded",
    body: "Dynamic checks follow the configured sandbox and remain authorization-first and remediation-oriented.",
  },
  {
    title: "Remediation with momentum",
    stat: "Actionable",
    body: "Confirmed issues include severity context, evidence, and fix guidance that engineers can act on.",
  },
  {
    title: "Designed for busy teams",
    stat: "One console",
    body: "Repository intake, findings, scorecards, and exports stay in one operational surface.",
  },
];
...
const PIPELINE = [
  {
    id: "01",
    title: "Intake and authorization check",
    body: "Connect a repository through the protected console and confirm the review boundary before starting.",
    output: "Target + branch context",
  },
  {
    id: "02",
    title: "Static code review",
    body: "Review dependency risk, secret/config exposure, auth/session patterns, and framework-specific code risk.",
    output: "Candidate findings",
  },
  {
    id: "03",
    title: "Sandbox-controlled validation",
    body: "Use the backend-configured sandbox path when supported to reduce noise without expanding the safety boundary.",
    output: "Evidence notes",
  },
  {
    id: "04",
    title: "Risk and severity scoring",
    body: "Prioritize confirmed findings with readable severity labels, confidence, and business impact.",
    output: "Prioritized queue",
  },
  {
    id: "05",
    title: "Executive-ready delivery",
    body: "Generate remediation-focused reports for founders, operators, and engineering stakeholders.",
...
export default function LandingPage() {
  ...
  const consoleLines: TerminalLine[] = CONSOLE_LINES;
      ...
      <div className={styles.container}>
        <motion.nav variants={fadeInUp} initial="hidden" animate="visible" className={styles.nav} aria-label="Primary navigation">
          ...
          <div className={styles.navLinks}>
            ...
            <motion.div whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.98 }}>
              <Link href={isLoggedIn ? "/dashboard" : "/signin"} className={styles.navCta}>{isLoggedIn ? "Dashboard" : "Open Console"}</Link>
            </motion.div>
          </div>
        </motion.nav>
        ...
        <section className={styles.hero}>
          <motion.div variants={staggerContainer} initial="hidden" animate="visible" className={styles.heroCopy}>
            ...
            <motion.h1 variants={fadeInUp} className={styles.heroTitle}>
              Agentic security audits for <span className="hero-gradient-text">modern SaaS teams.</span>
            </motion.h1>
            <motion.p variants={fadeInUp} className={styles.heroBody}>
              Run authorization-only audits, review evidence-backed findings, and generate founder-ready security reports.
            </motion.p>
            ...
            <motion.div variants={fadeInUp} className={styles.heroActions}>
              <motion.div whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
                <Link href={isLoggedIn ? "/dashboard" : "/signin"} className={cx(styles.primaryButton, "hero-primary-btn")}>
                  Start Audit <ArrowRight size={16} className="hero-cta-arrow" />
                </Link>
              </motion.div>
              <motion.div whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
                <Link href="/dashboard" className={styles.secondaryButton}>View Reports</Link>
              </motion.div>
            </motion.div>

            <motion.aside variants={fadeInUp} className={styles.heroNote}>
              <span className={styles.heroNoteLabel}>Safety note</span>
              <p>Only audit systems you own or are authorized to test.</p>
            </motion.aside>

            <motion.div variants={fadeInUp} className={styles.heroFootnotes}>
              <span className={styles.heroFootnote}>Sandbox-first</span>
              <span className={styles.heroFootnote}>Authorization-only</span>
              <span className={styles.heroFootnote}>Evidence-backed</span>
              <span className={styles.heroFootnote}>Remediation-focused</span>
            </motion.div>
            ...
            <motion.div variants={staggerContainer} className={styles.heroMetrics}>
              {HERO_METRICS.map((metric) => {
                ...
                return (
                  <motion.article key={metric.label} variants={scaleUp} whileHover={{ scale: 1.02, borderColor: "rgba(255, 114, 0, 0.2)" }} className={styles.metricCard}>
          ...
          <motion.aside variants={fadeInUp} initial="hidden" animate="visible" className={styles.heroBoard} aria-label="Trust model preview">
            <div className={styles.boardHeader}>
              <div>
                <p className={styles.boardLabel}>Trust model</p>
                <h2 className={styles.boardTitle}>Built for defensive review workflows</h2>
              </div>
              <span className={styles.boardBadge}>Consent-based</span>
            </div>

            <motion.div variants={staggerContainer} className={styles.boardList}>
              {TRUST_CARDS.map((row, index) => (
                <motion.article key={row.label} variants={fadeInUp} whileHover={{ x: 4, background: "rgba(255, 255, 255, 0.05)" }} className={styles.boardItem}>
            ...
            <div className={styles.boardFooter}>
              <div>
                ...
                <p className={styles.boardFooterValue}>Trace logs, finding evidence, severity ranking, and a remediation handoff package.</p>
              </div>
              <motion.div whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
                <Link href={isLoggedIn ? "/dashboard" : "/signin"} className={styles.boardFooterLink}>{isLoggedIn ? "Go to Dashboard" : "Launch workspace"}</Link>
              </motion.div>
            </div>
          </motion.aside>
        </section>
        ...
        <motion.section id="capabilities" initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} variants={staggerContainer} className={styles.section}>
          <motion.div variants={fadeInUp} className={styles.sectionHeader}>
            <div>
              <p className={styles.eyebrow}>Platform posture</p>
              <h2 className={styles.sectionTitle}>Less dashboard theater. More useful context.</h2>
            </div>
            <p className={styles.sectionIntro}>The UI explains the audit boundary, the evidence, and the remediation path in language a SaaS team can act on.</p>
          </motion.div>

          <div className={styles.capabilityGrid}>
            {CAPABILITIES.map((item) => (
              <motion.article key={item.title} variants={fadeInUp} whileHover={{ y: -4, borderColor: "rgba(255,114,0,0.2)", background: "rgba(255,255,255,0.03)" }} className={styles.capabilityCard}>
                <div className={styles.capabilityMeta}><span>{item.stat}</span></div>
                <h3>{item.title}</h3>
                <p>{item.body}</p>
              </motion.article>
            ))}
        ...
        <motion.section initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} variants={staggerContainer} className={cx(styles.section, styles.hintsSection)}>
          <motion.div variants={fadeInUp} className={styles.sectionHeader}>
            ...
            <p className={styles.sectionIntro}>Use a familiar repository, keep scope narrow, and review the output with someone who knows the code.</p>
          </motion.div>

          <div className={styles.hintGrid}>
            {FIRST_RUN_HINTS.map((hint) => (
              <motion.article key={hint.title} variants={fadeInUp} whileHover={{ y: -4, borderColor: "rgba(255,184,0,0.25)", background: "rgba(255,255,255,0.03)" }} className={styles.hintCard}>
        ...
        <motion.section id="workflow" initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} variants={staggerContainer} className={cx(styles.section, styles.demoSection)}>
          <motion.div variants={fadeInUp} className={styles.sectionHeader}>
            <div>
              ...
              <h2 className={styles.sectionTitle}>From authorized intake to remediation report.</h2>
            </div>
            <p className={styles.sectionIntro}>The landing page no longer simulates audit results. Real audits start only inside the authenticated console.</p>
          </motion.div>

          <div className={styles.demoSplit}>
            <div className={styles.pipelinePanel}>
              {PIPELINE.map((step) => (
                <motion.article key={step.id} variants={fadeInLeft} className={cx(styles.pipelineCard, "hero-pipeline-step")} layout>
                  <div className={styles.pipelineHeader}>
                    <span>{step.id}</span>
                    <strong style={{ display: "flex", alignItems: "center", gap: 6 }}>{step.output}</strong>
                  </div>
                  <h3>{step.title}</h3>
                  <p>{step.body}</p>
                </motion.article>
            ...
            <motion.div variants={fadeInRight} className={styles.simulator}>
              <div className={styles.simulatorHeader}>
                ...
                <span className={styles.simulatorTitle}>firecrow@maestro / safety-brief</span>
              </div>

              <div className={styles.simulatorBody}>
                {consoleLines.map((line, index) => (
                  <motion.div className={styles.terminalLine} key={`${line.text}-${index}`} initial={{ opacity: 0, y: 12, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1 }} transition={{ duration: 0.25, ease: "easeOut" }}>
                    <span className={styles.terminalPrompt}>&gt;</span>
                    <span className={cx(styles.terminalText, line.tone === "info" && styles.infoTone, line.tone === "success" && styles.successTone, line.tone === "warning" && styles.warningTone, line.tone === "error" && styles.errorTone)}>{line.text}</span>
                  </motion.div>
                ))}
              </div>

              <div className={styles.simulatorFooter}>
                <Link href={isLoggedIn ? "/dashboard" : "/signin"} className={styles.launchButton}>Start an authorized audit</Link>
              </div>
            </motion.div>
          </div>
        </motion.section>
        ...
        <motion.section id="agents" initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} variants={staggerContainer} className={styles.section}>
          <motion.div variants={fadeInUp} className={styles.sectionHeader}>
            <div>
              ...
              <h2 className={styles.sectionTitle}>Each role has one clear job.</h2>
            </div>
            <p className={styles.sectionIntro}>Readable responsibilities keep the security workflow understandable for founders, operators, and engineers.</p>
          </motion.div>

          <div className="agents-showcase-grid">
            {AGENTS.map((agent, index) => (
              <motion.article key={agent.id} variants={fadeInUp} className="agent-showcase-card">
                <div className="agent-showcase-header">
                  <span className="agent-idx">{String(index + 1).padStart(2, "0")}</span>
                  <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                    <span className="agent-status-dot" />
                    <span className="agent-idx">Ready</span>
                  </div>
                </div>
                <h3>{agent.id}</h3>
        ...
        <motion.section initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} variants={fadeInUp} className="cta-band">
          <div className="cta-band-content">
            <p className={styles.eyebrow}>Next step</p>
            <h2>Run it on a repo your team already knows.</h2>
            <p>That first pass is where trust gets built. Keep the scope familiar, keep the language human, and the review moves faster.</p>
            <div className={styles.heroActions} style={{ marginTop: 24 }}>
              <motion.div whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
                <Link href={isLoggedIn ? "/dashboard" : "/signin"} className={styles.primaryButton}>{isLoggedIn ? "Go to Dashboard" : "Go to sign in"}</Link>
              </motion.div>
              <motion.div whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
                <PolicyLink href="/privacy-policy" policy="privacy_policy" source="landing_cta" className={styles.secondaryButton}>Review privacy policy</PolicyLink>
              </motion.div>
            </div>
          </div>
        </motion.section>
        ...
        <footer className={styles.footer}>
          <div className={styles.footerBrand}>
            ...
            <p>Repository intake, validation, severity scoring, and remediation handoff in a single security workflow.</p>
          </div>
          ...
          <div className={styles.footerLinks}>
            ...
            <PolicyLink href="/privacy-policy" policy="privacy_policy" source="landing_footer">Privacy</PolicyLink>
          </div>
        </footer>
      </div>
    </main>
